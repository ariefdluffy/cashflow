...
export const users = derived(transactions, ($tx) => {
...
export const stats = derived(filteredTransactions, ($tx) => {
  ...
  const largestExpense =
    expense.length > 0 ? Math.max(...expense.map((t) => t.nominal)) : 0;
...
// Derived: saldo berjalan (cumulative balance sampai akhir filter range)
export const runningBalance = derived(
  [transactions, filterStore],
  ([$transactions, $filterStore]) => {
    const { endDate } =
      $filterStore.preset !== "custom"
    ...
    for (const t of sorted) {
      bal += t.jenis === "Masuk" ? t.nominal : -t.nominal;
    }
    return bal;
  },
);
...
export const chartData = derived(filteredTransactions, ($tx) => {
...
export const categoryData = derived(filteredTransactions, ($tx) => {
  const expense = $tx.filter((t) => t.jenis === "Keluar");
...
export const activityLog = derived(transactions, ($tx) => {
  return [...$tx]
...
export const forecast = derived(transactions, ($tx) => {
  ...
  const avgNet = Math.round(
    dailyNet.reduce((s, v) => s + v, 0) / dailyNet.length,
  );
  const lastBalance =
    sorted
      .filter((t) => t.jenis === "Masuk")
...
export const burnRate = derived(transactions, ($tx) => {
  const expense = $tx.filter((t) => t.jenis === "Keluar");
  const income = $tx.filter((t) => t.jenis === "Masuk");
  if (expense.length === 0)
    return {
...
export const topCategories = derived(categoryData, ($cat) => $cat.slice(0, 5));

// Derived: income category breakdown (baru — gak ditampilkan sebelumnya)
export const incomeCategoryData = derived(filteredTransactions, ($tx) => {
  const income = $tx.filter((t) => t.jenis === "Masuk");
  const grouped = new Map<string, number>();
  let total = 0;
...
export const categoryComparison = derived(filteredTransactions, ($tx) => {
  const incomeMap = new Map<string, number>();
  const expenseMap = new Map<string, number>();
  for (const t of $tx) {
    if (t.jenis === "Masuk")
      incomeMap.set(t.kategori, (incomeMap.get(t.kategori) || 0) + t.nominal);
    else
      expenseMap.set(t.kategori, (expenseMap.get(t.kategori) || 0) + t.nominal);
  }
...
export const monthlyTrend = derived(transactions, ($tx) => {
  const grouped = new Map<string, { income: number; expense: number }>();
